...
const getters = {
  ...
  isAdminRole: (state, getters) => {
    if (getters.userInfo.roleList) {
      return (
        getters.userInfo.roleList.indexOf(USER_TYPE.ADMIN) != -1 ||
...
const mutations = {
  ...
  substractUnreadMessageCount(state, { needSubstractMsg }) {
    // 负数也没关系
    state.unreadMessage[needSubstractMsg.name] =
      state.unreadMessage[needSubstractMsg.name] - needSubstractMsg.num;
  },
