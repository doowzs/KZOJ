...
const getters = {
  ...
  isContestAdmin: (state, getters, _, rootGetters) => {
    return (
      ...
      (state.contest.author === rootGetters.userInfo.username ||
        rootGetters.isSuperAdmin ||
        state.groupContestAuth == 5)
    );
  },
  ...
  canSubmit: (state, getters) => {
    return (
      state.intoAccess ||
      state.submitAccess ||
      state.contest.auth === CONTEST_TYPE.PUBLIC ||
  ...
  BeginToNowDuration: (state, getters) => {
    return moment
      .duration(state.now.diff(getters.contestStartTime, "seconds"), "seconds")
      .asSeconds();
  },
...
const mutations = {
  ...
  changeContestProblems(state, payload) {
    ...
    for (var j = 0, len = payload.contestProblems.length; j < len; j++) {
      tmp[payload.contestProblems[j].displayId] =
        payload.contestProblems[j].color;
    }
    ...
    return new Promise((resolve, reject) => {
      api
        .getContestProblemList(
          rootState.route.params.contestID,
          rootState.contest.isContainsAfterContestJudge,
        )
        .then(
          (res) => {
            resolve(res);
