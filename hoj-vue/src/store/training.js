...
const getters = {
  isTrainingAdmin: (state, getters, _, rootGetters) => {
    return (
      ...
      (state.training.author === rootGetters.userInfo.username ||
        rootGetters.isSuperAdmin ||
        state.groupTrainingAuth == 5)
    );
  },
  ...
  trainingPasswordFormVisible: (state, getters) => {
    ...
    return (
      !state.intoAccess &&
      state.training.auth != TRAINING_TYPE.Public.name &&
      !getters.isTrainingAdmin
    );
  },
};
...
const actions = {
  getTraining({ commit, rootState, dispatch }) {
    return new Promise((resolve, reject) => {
      api.getTraining(rootState.route.params.trainingID).then(
        (res) => {
          resolve(res);
  ...
  getTrainingProblemList({ commit, rootState }) {
    return new Promise((resolve, reject) => {
      api.getTrainingProblemList(rootState.route.params.trainingID).then(
        (res) => {
          resolve(res);
  ...
  getTrainingAccess({ commit, rootState }, trainingType) {
    return new Promise((resolve, reject) => {
      api
        .getTrainingAccess(rootState.route.params.trainingID)
        .then((res) => {
  ...
  getGroupTrainingAuth({ commit, rootState }, gid) {
    return new Promise((resolve, reject) => {
      api
        ...
        .then((res) => {
          commit("changeGroupTrainingAuth", {
            groupTrainingAuth: res.data.data,
          });
          resolve(res);
        })
        .catch();
