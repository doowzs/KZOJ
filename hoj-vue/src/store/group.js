...
const getters = {
  ...
  isGroupOwner: (state, getters, _, rootGetters) => {
    return (
      ...
      state.group.owner == getters.userInfo.username
    );
  },
  ...
  isGroupAdmin: (state, getters, _, rootGetters) => {
    return (
      rootGetters.isAuthenticated && (getters.isGroupRoot || state.auth === 4)
    );
  },
  isGroupMember: (state, getters, _, rootGetters) => {
    return (
      rootGetters.isAuthenticated && (getters.isGroupAdmin || state.auth === 3)
    );
  },
...
const actions = {
  ...
  getGroup({ commit, rootState, dispatch }) {
    return new Promise((resolve, reject) => {
      api.getGroup(rootState.route.params.groupID).then(
        (res) => {
          resolve(res);
  ...
  getGroupAccess({ commit, rootState }) {
    return new Promise((resolve, reject) => {
      api
        .getGroupAccess(rootState.route.params.groupID)
        .then((res) => {
  ...
  getGroupAuth({ commit, rootState }) {
    return new Promise((resolve, reject) => {
      api
        .getGroupAuth(rootState.route.params.groupID)
        .then((res) => {
