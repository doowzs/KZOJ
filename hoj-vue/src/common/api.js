  ...
  (error) => {
    ...
    if (error.response) {
      ...
      switch (error.response.status) {
        ...
        case 404:
          mMessage.error(
            i18n.t("m.Query_error_unable_to_find_the_resource_to_request"),
          );
          break;
        // 其他错误，直接抛出错误提示
        default:
      ...
      if (error.code == "ECONNABORTED" || error.message.includes("timeout")) {
        ...
        mMessage.error(
          i18n.t(
            "m.Network_error_abnormal_link_with_server_please_try_again_later",
          ),
        );
      }
      return Promise.reject(error);
  ...
  getProblemList(searchParams) {
    ...
    Object.keys(searchParams).forEach((element) => {
      if (
        ...
        searchParams[element] !== null &&
        searchParams[element] !== undefined
      ) {
